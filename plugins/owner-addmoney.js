let handler = async (m, { conn, command, text, args }) => {
    ...
    // Determining whether to add or delove money based on the command
    if (command === 'addmoney') {
        ...
        if (moneyValue > users[user].money) {
            ...
            // deloving the specified money from the user's account
            users[user].money -= moneyValue
...
handler.help = ['addmoney', 'delmoney']
handler.tags = ['owner']
handler.command = /^(add|del)money$/i
handler.rowner = true
handler.group = true
