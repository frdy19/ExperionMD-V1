...
`.trim()
                ...
                await this.adReply(room.player1, str2, '- ᴅ ᴜ ɴ ɢ ᴇ ᴏ ɴ -', '', fs.readFileSync('./media/' + dungimg.getRandom()), '', m)
                if (mythic > 0) {
...
`.trim()
            ...
            for (let _p of PLAYER) {
                room.price.money += (Math.floor(Math.random() * 5000)) * 1
                room.price.exp += (Math.floor(Math.random() * 6000)) * 1
                room.game.sampah += (Math.floor(Math.random() * 160000)) * 1
                room.price.string += (pickRandom([0, 0, 5, 100, 30, 400, 0, 1, 0, 0, 0, 0, 0, 0])) * 1
                room.price.kayu += (pickRandom([0, 900, 0, 1, 10, 4, 50, 0, 0, 0, 0, 0, 0])) * 1
                room.price.batu += (pickRandom([0, 0, 70, 5, 100, 83, 4, 1, 0, 30, 0, 0, 0, 0])) * 1
                room.game.common += (pickRandom([0, 80, 0, 243, 4, 96, 1, 0, 90, 0, 0, 60, 60, 0, 0])) * 1
            }
...
`.trim()
                let dungimg = [ 'dungeon1.jpg', 'dungeon2.jpg', 'dungeon3.jpg' ]
                await this.adReply(c1, str2, '- ᴅ ᴜ ɴ ɢ ᴇ ᴏ ɴ -', '', fs.readFileSync('./media/' + dungimg.getRandom()), '', m)
